...
export function HistorySidebar({
  ...
  const historySites = generatedSites.slice(0, 8);

  if (!historySites.length && !recentProspects.length) return null;
  ...
  return (
    <section className="rounded-2xl border border-[oklch(0.88_0.03_90)] bg-white p-5 shadow-sm">
      {!!historySites.length && (
        <div>
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-[oklch(0.25_0.02_50)]">
              Recent Sites
            </h3>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {historySites.map((site) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="group block rounded-xl border border-[oklch(0.9_0.02_90)] p-3 transition hover:border-[oklch(0.48_0.12_155)]"
              >
      ...
      {!!recentProspects.length && (
        <div className={historySites.length ? "mt-6 border-t border-[oklch(0.9_0.02_90)] pt-5" : ""}>
          <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-stone-400">
            Recently Loaded Prospects
          </p>
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            {recentProspects.map((prospect) => (
              <button
                ...
                className="rounded-xl bg-[oklch(0.985_0.005_90)] px-4 py-3 text-left text-xs text-stone-600 transition hover:bg-[oklch(0.95_0.02_90)]"
              >
